...
def print_operation_table(operation, num_rows=6, num_columns=6):
    for x in range(1, num_rows + 1):
        ...
        for y in range(1, num_columns + 1):
            num = operation(x, y)
            nums.append(num)
        print(*[str(x) for x in nums], sep='\t')

    print('', sep='\n')

# Вариант 2 (с использованием функций высшего порядка):
    nums = []
    [list(map(lambda y: nums.append(operation(x, y)), range(1, num_columns + 1))) for x in range(1, num_rows + 1)]
    elements = list(zip(*[iter(nums)]*num_columns))
    [print(*elements[i], sep='\t') for i in range(num_rows)]
